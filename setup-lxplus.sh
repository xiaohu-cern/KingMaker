...
action() {
    ...
    # THIS is dedicated lxplus setup
    ...
    ENV_NAMES=($(grep '^ENV_NAME' lawluigi_configs/${ANA_NAME}_lxplus_luigi.cfg | sed 's@ENV_NAME\s*=\s*\([^\s]*\)\s*@\1@g'))
    ...
    for ENV_NAME in ${ENV_NAMES[@]}; do
        ...
        if [[ -d "/cvmfs/etp.kit.edu/LAW_envs/conda_envs/miniconda/envs/${ENV_NAME}" ]]; then
            ...
            # lxplus has a default conda, skip it and install a local miniconda
            if command -v conda &> /dev/null; then
    ...
    case ${ANA_NAME} in
        KingMaker)
            ...
            if [ ! -d CROWN ]; then
                ...
                #199    if(PACKAGE_VERSION VERSION_LESS MIN_VERSION)
                #200        message(FATAL_ERROR "The version of Python package ${PYPINAME} is too old (found ${PACKAGE_VERSION}, require at least ${MIN_VERSION}).")
                #201    endif()
                sed -i '199,201 {s/^/#/}' CROWN/CMakeLists.txt
            fi
    ...
    export LAW_CONFIG_FILE="${BASE_DIR}/lawluigi_configs/${ANA_NAME}_lxplus_law.cfg"
    export LUIGI_CONFIG_PATH="${BASE_DIR}/lawluigi_configs/${ANA_NAME}_lxplus_luigi.cfg"
    ...
    # tasks
    _addpy "${BASE_DIR}/processor-lxplus"
    _addpy "${BASE_DIR}/processor-lxplus/tasks"

    # add voms proxy path
    # proxy at lxplus afs
    #voms-proxy-init --rfc --voms cms -valid 192:00 --out x509up_u$(id -u)
    export X509_USER_PROXY=${BASE_DIR}/x509up_u$(id -u)
