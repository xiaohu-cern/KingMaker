...
class HTCondorWorkflow(Task, law.htcondor.HTCondorWorkflow):
    ...
    # at lxplus save proxy file at KingMaker root dir and set X509_USER_PROXY
    ...
    def htcondor_job_config(self, config, job_num, branches):
        ...
        #    ("accounting_group", self.htcondor_accounting_group)
            ("+AccountingGroup", self.htcondor_accounting_group) # lxplus
        )
        ...
        #    config.custom_content.append(("Requirements", self.htcondor_requirements)) # should be +Requirements for lxplus
        #config.custom_content.append(("+RemoteJob", self.htcondor_remote_job)) # not for lxplus
        config.custom_content.append(("universe", self.htcondor_universe))
        #config.custom_content.append(("docker_image", self.htcondor_docker_image)) # not for lxplus atm
        #config.custom_content.append(("+RequestWalltime", self.htcondor_walltime)) # not for lxplus
        #config.custom_content.append(("Proxy_path", self.htcondor_user_proxy)) # not sure working at lxplus
        config.custom_content.append(("use_x509userproxy", "true")) # recommended by AAA for lxplus
        config.custom_content.append(("x509userproxy", self.htcondor_user_proxy)) # together with the above for lxplus
        ...
        #config.custom_content.append(("RequestMemory", self.htcondor_request_memory)) # not for lxplus
        #config.custom_content.append(("RequestDisk", self.htcondor_request_disk)) # not for lxplus
        ...
        if not tarball.exists():
            ...
            command = [
                ...
                "processor-lxplus",
                "lawluigi_configs/{}_lxplus_luigi.cfg".format(analysis_name),
                "lawluigi_configs/{}_lxplus_law.cfg".format(analysis_name),
                "law",
